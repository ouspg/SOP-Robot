...
class HandGestureNode(Node):
    def __init__(self):
        ...
        self.left_hand_action_client = ActionClient(
            self, FollowJointTrajectory, 'l_hand_controller/follow_joint_trajectory'
        )
        self.right_hand_gesture_subscription = self.create_subscription(
            String, '/r_hand/r_hand_topic', self.r_hand_callback, 10
        )
        self.left_hand_gesture_subscription = self.create_subscription(
            String, '/l_hand/l_hand_topic', self.l_hand_callback, 10
        )
    ...
    def send_left_hand_goal(self, action, duration=None):
        if duration is None:
            duration = Duration(sec=1)
        goal_msg = FollowJointTrajectory.Goal()
        trajectory_points = JointTrajectoryPoint(positions=action, time_from_start=duration)
