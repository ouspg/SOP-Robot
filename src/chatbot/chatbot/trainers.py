...
class ChatterBotJSONTrainer(Trainer):
    """
    training the chatbot with json data in the specific format used in ambignq data
    """

    def train(self, *training_data_paths, input_language):
                    ...
                    for annotation in annotations:
                        if annotation['type'] == 'multipleQAs':
                            for qa_pair in annotation['qaPairs']:
                                ...
                                question_statement = self.get_preprocessed_statement(question_statement)
                                statements_to_create.append(question_statement)
                                ...
                                for answer in answers:
                                    ...
                                    answer_statement = self.get_preprocessed_statement(answer_statement)
                                    statements_to_create.append(answer_statement)

            self.chatbot.storage.create_many(statements_to_create)

class SQuADJSONTrainer(Trainer):
    """
    Train the chatbot with SQuAD dataset in JSON format
    """
...
class MSMARCOJSONTrainer(Trainer):
    #train with msmarco data



    def train(self, *training_data_paths, input_language, chunk_size=1000):
        for training_data_path in training_data_paths:
            print(f"Processing file: {training_data_path}")
            with open(training_data_path, 'r') as file:
                chunk_counter = 0
                statements_to_create = []
                ...
                for line_number, line in enumerate(file):
                    ...
                    if 'query' in data and 'answers' in data:
                        ...
                        for answer in answers:
                            ...
                            query_statement = self.get_preprocessed_statement(query_statement)
                            statements_to_create.append(query_statement)
                    ...
                    if chunk_counter % chunk_size == 0:
                        print(f"Storing statements, length:")
                        self.chatbot.storage.create_many(statements_to_create)
                        statements_to_create = []
            ...
            with open(training_data_path, 'r', encoding='ISO-8859-1') as file:
                ...
                for row in reader:
                    if len(row) == 6:
                        ...
                        print(f"Warning: Skipping row with {len(row)} columns, expected 6 columns: {row}")
