...
#include "dynamixel-controllers/controller.h"
...
{
  get_parameter_or<bool>("use_joint_states_topic", is_joint_state_topic_, true);
  get_parameter_or<bool>("use_cmd_vel_topic", is_cmd_vel_topic_, false);
  get_parameter_or<bool>("use_moveit", use_moveit_, false);

  get_parameter_or<double>("dxl_read_period", read_period_, 0.010f);
  get_parameter_or<double>("dxl_write_period", write_period_, 0.010f);
  get_parameter_or<double>("publish_period", pub_period_, 0.010f);

  if (is_cmd_vel_topic_)
  {
    get_parameter_or<double>("mobile_robot_config/seperation_between_wheels", wheel_separation_, 0.0);
    get_parameter_or<double>("mobile_robot_config/radius_of_wheel", wheel_radius_, 0.0);
  }

  // Todo: use shared (or uniq) pointers
  jnt_tra_ = new JointTrajectory;
...
bool DynamixelController::init(const std::string port_name, const uint32_t baud_rate)
{
  bool result = false;

  std::string yaml_file = get_parameter("dynamixel_info").as_string();
  ...
  if (result == false)
  {
    printf("Please check Dynamixel ID or BaudRate");
    return false;
  }
  ...
  if (result == false)
  {
    printf("Please check control table (http://emanual.robotis.com/#control-table)");
    return false;
  }
  ...
  if (result == false)
  {
    printf("Please check control items");
    return false;
  }
  ...
  if (result == false)
  {
    printf("Failed to set Dynamixel SDK Handler");
    return false;
  }
  ...
  // dynamixel command server disabled for now. ROS2 dynamixel workbench does not have the command message defs for some reason
  // this->initServer();

  read_timer_ = create_wall_timer(duration<double>(getReadPeriod()), std::bind(&DynamixelController::readCallback, this));
  write_timer_ = create_wall_timer(duration<double>(getWritePeriod()), std::bind(&DynamixelController::writeCallback, this));
  publish_timer_ = create_wall_timer(duration<double>(getPublishPeriod()), std::bind(&DynamixelController::publishCallback, this));
  return true;
}
...
bool DynamixelController::loadDynamixels(void)
{
  ...
  for (auto const &dxl : dynamixel_)
  {
    ...
    {
      printf("%s", log);
      printf("Can't find Dynamixel ID '%d'", dxl.second);
      return result;
    }
    else
    {
      printf("Name : %s, ID : %d, Model Number : %d", dxl.first.c_str(), dxl.second, model_number);
    }
  }
    ...
    for (auto const &info : dynamixel_info_)
    {
      if (dxl.first == info.first)
      {
        if (info.second.item_name != "ID" && info.second.item_name != "Baud_Rate")
        {
          ...
          {
            printf("%s", log);
            printf("Failed to write value[%d] on items[%s] to Dynamixel[Name : %s, ID : %d]", info.second.value, info.second.item_name.c_str(), dxl.first.c_str(), dxl.second);
            return false;
          }
        }
      }
...
void DynamixelController::initPublisher()
{
  ...
  dynamixel_state_list_pub_ = this->create_publisher<dynamixel_workbench_msgs::msg::DynamixelStateList>("dynamixel_state", 100);
...
void DynamixelController::initSubscriber()
{
  ...
  trajectory_sub_ = create_subscription<trajectory_msgs::msg::JointTrajectory>(
    "joint_trajectory", 100, std::bind(&DynamixelController::trajectoryMsgCallback, this, _1));

  if (is_cmd_vel_topic_) {
    cmd_vel_sub_ = create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", 10, std::bind(&DynamixelController::commandVelocityCallback, this, _1));
  }
}
...
void DynamixelController::commandVelocityCallback(const geometry_msgs::msg::Twist::ConstPtr msg)
{
  bool result = false;
  const char *log = NULL;
...
void DynamixelController::trajectoryMsgCallback(const trajectory_msgs::msg::JointTrajectory::ConstPtr msg)
{
  uint8_t id_cnt = 0;
  bool result = false;
  WayPoint wp;
  ...
  if (is_moving_ == false)
  {
    ...
    {
      printf("'%s' is ready to move", joint.c_str());
    ...
    if (id_cnt != 0)
    {
      ...
      while (cnt < msg->points.size())
      {
        ...
        else
        {
          ...
          if (cnt == 0)
            move_time = msg->points[cnt].time_from_start.sec;
          else
            move_time = msg->points[cnt].time_from_start.sec - msg->points[cnt - 1].time_from_start.sec;
      ...
      printf("Succeeded to get joint trajectory!");
      is_moving_ = true;
    }
    else
    {
      printf("Please check joint_name");
    }
  }
  else
  {
    // printf_THROTTLE(1, "Dynamixel is moving");
  }
}
                                                      ...
                                                      dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
  ...
  {
    printf("%s", log);
    printf("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value, item_name.c_str(), id);
  }
...
int main(int argc, char **argv)
{
  ...
  if (!node->init(port_name, baud_rate))
  {
    return 1;
  }
