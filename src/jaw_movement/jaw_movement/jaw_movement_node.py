...
class JawMoverNode(Node):
    ...
    def __init__(self):
        ...
        self._action_client = ActionClient(
            self, FollowJointTrajectory, '/jaw_controller/follow_joint_trajectory'
        )

        # 'input' is a placeholder for subscribing to the speech-synthesis topic.
    ...
    def timer_callback(self):
        ...
        if len(self.input) > 0:
            ...
            trajectory_points = JointTrajectoryPoint(
                positions=[self.jawPos], time_from_start=self.charDuration
            )
            goal_msg.trajectory = JointTrajectory(
                joint_names=['head_jaw_joint'], points=[trajectory_points]
            )
