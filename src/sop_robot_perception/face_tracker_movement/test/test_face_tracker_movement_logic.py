...
import pytest
from face_tracker_movement.face_tracker_movement_node import FaceTrackerMovementNode

def build_node():
    node = FaceTrackerMovementNode.__new__(FaceTrackerMovementNode)
    ...
    object.__setattr__(node, "logger", SimpleNamespace(info=lambda *_: None))
    return node
...
def test_eyes_state_callback_updates_camera_space_state():
    ...
    assert node.eyes_state == pytest.approx([-0.2, -0.05])
