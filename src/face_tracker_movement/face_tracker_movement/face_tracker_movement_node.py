...
class FaceTrackerMovementNode(Node):
    def __init__(self):
        ...
        functionality = (
            self.declare_parameter('functionality', 'full').get_parameter_value().string_value
        )

        simulation = self.declare_parameter('simulation', False).get_parameter_value()._bool_value

        self.logger = self.get_logger()

        # Action clients for eye and head controllers
        self.eye_action_client = ActionClient(
            self, FollowJointTrajectory, '/eyes_controller/follow_joint_trajectory'
        )
        ...
        self.face_list_subscription = self.create_subscription(
            Faces, '/face_tracker/faces', self.face_list_callback, 2
        )
        self.head_state_subscription = self.create_subscription(
            JointTrajectoryControllerState,
            '/head_controller/controller_state',
            self.head_state_callback,
            5,
        )
        self.eyes_state_subscription = self.create_subscription(
            JointTrajectoryControllerState,
        ...
        self.head_gesture_subscription = self.create_subscription(
            String, '/face_tracker_movement/head_gesture_topic', self.head_gesture_callback, 10
        )

        self.focused_face_publisher = self.create_publisher(
            Face, '/face_tracker_movement/focused_face', 10
        )
        ...
        if simulation:
            ...
            # starting states
            self.start_head_state: list[float] = [
                0.0,
                0.0,
                0.0,
                1.0,
            ]  # Should not be modified in runtime.
            self.eyes_center_position: list[float] = [
                0.0,
                0.0,
            ]  # Should not be modified in runtime.
            ...
            # starting states
            self.start_head_state = [0.6, 0.5, -0.5, 1.2]  # Should not be modified.
            self.eyes_center_position = [-0.7, -0.75]  # Should not be modified.
        ...
        self.angle_per_pixel = self.camera_diagonal_fov / math.sqrt(
            self.camera_resolution_x**2 + self.camera_resolution_y**2
        )
        ...
        self.camera_angle_eye_vertical_lower_limit = (
            self.eye_vertical_lower_limit / self.camera_angle_eye_vertical_coeff
        )
        self.camera_angle_eye_vertical_upper_limit = (
            self.eye_vertical_upper_limit / self.camera_angle_eye_vertical_coeff
        )
        self.camera_angle_eye_horizontal_lower_limit = (
            self.eye_horizontal_lower_limit / self.camera_angle_eye_horizontal_coeff
        )
        self.camera_angle_eye_horizontal_upper_limit = (
            self.eye_horizontal_upper_limit / self.camera_angle_eye_horizontal_coeff
        )
        ...
        # Servo IDs follow the head_controller joint order: pan, right tilt,
        # left tilt, and vertical tilt.
        self.head_joint_ids = [4, 1, 3, 2]
        self.head_state = self.start_head_state[
            :
        ]  # Tries to have the up-to-date head servo values.

        # Servo IDs follow the eyes_controller joint order: horizontal, then vertical.
        self.eyes_joint_ids = [9, 11]
        self.eyes_servo_state = self.eyes_center_position[
            :
        ]  # Tries to have the up-to-date head servo values.
        self.eyes_state = [
        ...
        self.preferred_face_tracking_start_time: float | None = None
    ...
    def face_list_callback(self, msg):
        ...
        # TODO: This likely need not run at 30 FPS; consider moving it to its own timer.
        ...
        if face:
            ...
            # Chance of a glance on each frame where a face is detected.
            # TODO: Decide on a good value.
    ...
    def head_state_callback(self, msg):
        for i, val in enumerate(msg.actual.positions):
            if math.isnan(val):
                ...
                self.logger.warning(
                    'Head joint ID ' + str(self.head_joint_ids[i]) + ' is not responding',
    ...
    def eyes_state_callback(self, msg):
        for i, val in enumerate(msg.actual.positions):
            if math.isnan(val):
                ...
                self.logger.warning(
                    'Eye joint ID ' + str(self.eyes_joint_ids[i]) + ' is not responding',
                    throttle_duration_sec=5.0,
                )
            else:
                # Servo angle
    ...
    def idle_timer_callback(self):
        ...
        self.goal_pan = random.uniform(
            max(self.head_pan_lower_limit, self.head_state[0] - max_idle_movement),
            min(self.head_pan_upper_limit, self.head_state[0] + max_idle_movement),
        )

        # Travel distance is the current head position minus the goal position.
        # For example: current=-0.2, goal=-0.6, distance=abs(-0.2 - -0.6)=0.4.
        ...
        # Scale movement time by the available travel distance. A full side-to-side
        # pan takes four seconds, with a minimum movement time of 0.5 seconds.
        ...
        eye_goal_vertical = random.gauss(
            self.eyes_center_position[1],
            (self.eyes_center_position[1] + self.eye_vertical_upper_limit) / 3,
        )

        if pan_travel > 0:
            eye_goal_horizontal = random.uniform(
                self.eye_horizontal_lower_limit, self.eye_horizontal_lower_limit / 6
            )
        elif pan_travel < 0:
            eye_goal_horizontal = random.uniform(
                self.eye_horizontal_upper_limit / 6, self.eye_horizontal_upper_limit
            )
        else:
            eye_goal_horizontal = random.uniform(
        ...
        if self.head_enabled:
            self.send_pan_and_vertical_tilt_goal(
                self.goal_pan, self.start_head_state[3], Duration(sec=0, nanosec=movement_time)
            )
        # Reset the idle timer to the movement length plus a natural-looking random
    ...
    def select_face_to_track(self, faces):
        ...
        # TODO: Also use information from other modules, such as whether someone speaks.
        ...
        if max_face_size > self.near_interaction_treshold:
            ...
            for face in faces:
                if (face.diagonal > self.ignore_small_faces_treshold) and (
                    face.face_id
                    not in [blocked_face['face_id'] for blocked_face in self.blocked_faces]
                ):
    ...
    def select_face_far_interaction(self, faces):
        if self.preferred_face_id:
            selected_face = next(
                (face for face in faces if face.face_id == self.preferred_face_id), None
            )
            # Track preferred face if
            if selected_face:
                tracking_start = self.preferred_face_tracking_start_time
                if (
                    tracking_start is None
                    or time.time() - tracking_start < self.preferred_face_tracking_timelimit
                ):
                    ...
                    if len(faces) > 1:
                        ...
                        face_list = [
                            face for face in faces if face.face_id != self.preferred_face_id
                        ]
    ...
    def update_blocked_faces(self):
        ...
        for blocked_face in self.blocked_faces:
            if blocked_face['block_start'] + blocked_face['block_duration'] < time.time():
                self.logger.debug(f'Removed face {blocked_face["face_id"]} from blocked faces')
            else:
    ...
    def remove_preferred_face(self):
        ...
        self.logger.debug(f'Preferred face removed and blocked: {self.preferred_face_id}')
        self.preferred_face_id = None
    ...
    def track_face(self, face):
        ...
        if self.eyes_enabled and self.head_enabled:
            ...
            pan_angle_head, horizontal_angle_eyes = (
                self.divide_horizontal_camera_angle_for_head_and_eyes(horizontal_angle)
            )
            vertical_angle_head, vertical_angle_eyes = (
                self.divide_vertical_camera_angle_for_head_and_eyes(vertical_angle)
            )
        ...
        if self.eyes_enabled and (horizontal_angle_eyes != 0 or vertical_angle_eyes != 0):
            eye_goal_horizontal, eye_goal_vertical = self.transform_camera_angle_to_eye_location(
                horizontal_angle_eyes, vertical_angle_eyes
            )
            self.send_eye_goal(eye_goal_horizontal, eye_goal_vertical)

        if self.head_enabled and (pan_angle_head != 0 or vertical_angle_head != 0):
            goal_pan, goal_vertical_tilt = self.transform_camera_angle_to_head_location(
                pan_angle_head, vertical_angle_head
            )
            self.send_pan_and_vertical_tilt_goal(goal_pan, goal_vertical_tilt)

        # TODO: Remember prior face coordinates for use after losing the current face.
    ...
    def divide_horizontal_camera_angle_for_head_and_eyes(self, horizontal_angle):
        ...
        If the face is centered but the eyes are not, center the eyes and move the
        head toward the target. Otherwise, use one third of the camera angle for the
        eyes, up to their limit, and use the remaining angle for the head.
        """
        head_pan_angle = 0
        eyes_horizontal_angle = 0
        # Divide horizontal angle for head and eyes
        """if abs(horizontal_angle) < math.pi / 36:
            # Head is in the center sector of 10 degrees. TODO: Adjust angle.
            ...
            eyes_horizontal_angle = (
                -self.eyes_state[0] * eye_centering_multiplier + horizontal_angle
            )
        ...
        if horizontal_angle > 0:
            # Left movement
            eyes_horizontal_angle = min(
                self.camera_angle_eye_horizontal_lower_limit - self.eyes_state[0],
                horizontal_angle / 3,
            )
        else:
            # Right movement
            eyes_horizontal_angle = max(
                self.camera_angle_eye_horizontal_upper_limit - self.eyes_state[0],
                horizontal_angle / 3,
            )
    ...
    def nod(self, magnitude=0.4, delay=0.5, duration_of_individual_movements=0.4):
        ...
        Takes two optional arguments:
            ...
            duration_of_individual_movements: Time for one movement to finish,
                in seconds. Should be less than delay.
        """
        if self.head_state:
            ...
            self.fixed_gaze_head_turn(
                'up', verticalTilt - verticalTilt_start, duration_of_individual_movements
            )
    ...
    def glance(self, delay=0.5, duration_of_individual_movements=None):
        ...
        """
        if self.head_state:
            eye_location_horizontal, eye_location_vertical = self.get_random_eye_location(
                distance_from_current_horizontal_position=0.5
            )
    ...
    def fixed_gaze_head_turn(self, direction, magnitude, duration=0.4):
        """
        Turn the head while rotating the eyes in the opposite direction.

        Decrease the movement magnitude if it would exceed the range of either
    ...
    def send_eye_goal(self, horizontal, vertical, duration=None):
        # Avoid eye lockups by allowing more time for longer movements.
        if duration is None:
        ...
        trajectory_points = JointTrajectoryPoint(
            positions=[
                horizontal * self.eye_horizontal_multiplicator,
                vertical * self.eye_vertical_multiplicator,
            ],
        ...
        self.logger.debug(f'Eye location x: {horizontal:f}, eye location y: {vertical:f}')

    def send_horizontal_tilt_goal(self, horizontalTilt):
        goal_msg = FollowJointTrajectory.Goal()
        trajectory_points = JointTrajectoryPoint(
            positions=[-horizontalTilt, horizontalTilt], time_from_start=Duration(sec=1, nanosec=0)
        )
        goal_msg.trajectory = JointTrajectory(
            joint_names=['head_tilt_left_joint', 'head_tilt_right_joint'],
            points=[trajectory_points],
        )
    ...
    def send_pan_and_vertical_tilt_goal(self, pan, verticalTilt, duration=None):
        ...
        self.logger.debug('Turning head to x: ' + str(pan) + ' y: ' + str(verticalTilt))
        goal_msg = FollowJointTrajectory.Goal()
        trajectory_points = JointTrajectoryPoint(
            positions=[
                pan * self.head_pan_multiplicator,
                verticalTilt * self.head_vertical_multiplicator,
            ],
            time_from_start=duration,
        )
        goal_msg.trajectory = JointTrajectory(
            joint_names=['head_pan_joint', 'head_tilt_vertical_joint'], points=[trajectory_points]
        )
    ...
    # Horizontal tilt is separate and slower because its joints can stick at speed.
    def send_head_goal(self, pan, verticalTilt, horizontalTilt):
        # TODO: Re-enable once the horizontal-tilt mechanism is functional:
        # self.send_horizontal_tilt_goal(horizontalTilt)
        # Wait so the new goal does not override one still in progress:
        # time.sleep(1)
    ...
    def transform_camera_angle_to_eye_location(self, horizontal_angle, vertical_angle):
        ...
        eye_location_x = (
            horizontal_angle * self.camera_angle_eye_horizontal_coeff + self.eyes_servo_state[0]
        )
        # Vertical eye movement
        eye_location_y = (
            vertical_angle * self.camera_angle_eye_vertical_coeff + self.eyes_servo_state[1]
        )

        eye_location_y = max(
            min(self.eye_vertical_upper_limit, eye_location_y), self.eye_vertical_lower_limit
        )
        eye_location_x = max(
            min(self.eye_horizontal_upper_limit, eye_location_x), self.eye_horizontal_lower_limit
        )
    ...
    def transform_camera_angle_to_head_location(self, horizontal_angle, vertical_angle):
        ...
        pan = max(
            min(self.head_pan_upper_limit, pan), self.head_pan_lower_limit
        )  # limit head values to reasonable values
        ...
        vertical_tilt = max(
            min(self.head_vertical_upper_limit, vertical_tilt), self.head_vertical_lower_limit
        )
    ...
    """
    Return random servo coordinates far enough from the current gaze for a glance.
    """

    def get_random_eye_location(self, distance_from_current_horizontal_position=0.0):
        ...
        if len(random_horizontal_list) == 0:
            distance = distance_from_current_horizontal_position
            self.logger.warning(
                f'Cannot get random eye position because distance {distance} is too large '
                'for the eye limits. Keeping the current horizontal position.'
            )
            random_horizontal_list.append(self.eyes_servo_state[0])
        random_horizontal = random.choice(random_horizontal_list)
        random_vertical = random.uniform(
