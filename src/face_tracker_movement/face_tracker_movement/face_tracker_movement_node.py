...
class FaceTrackerMovementNode(Node):
    def __init__(self):
        ...
        if simulation:
            ...
            self.logger.info(
                "Notice that the camera is not moved in simulation, when face is moveing. "
            ...
            # starting states
            self.start_head_state = [
                0.6,
                0.5,
                -0.5,
                1.2,
            ]  # Should not be modified in runtime.
            self.eyes_center_position = [
                -0.7,
                -0.75,
            ]  # Should not be modified in runtime.
        ...
        self.head_joint_ids = [
            ...
            3,
            2,
        ]  # Servo ids for head joints. Order comes from head_controller: [head_pan_joint, head_tilt_right_joint, head_tilt_left_joint, head_tilt_vertical_joint]
        ...
        self.eyes_joint_ids = [
            9,
            11,
        ]  # Servo ids for eye joints. Order comes from eyes_controller: [eyes_shift_horizontal_joint, eyes_shift_vertical_joint]
        ...
        self.blocked_faces: list[
            dict
        ] = []  # Includes face_id, block_start and block_duration
    ...
    def head_state_callback(self, msg):
        for i, val in enumerate(msg.feedback.positions):
            if math.isnan(val):
    ...
    def eyes_state_callback(self, msg):
        ...
        if len(pos) < 2:
            self.logger.warn(f"Expected 2 eye joints, got {len(pos)}")
            return

        # Ensure arrays exist and have length 2
        if not hasattr(self, "eyes_servo_state") or len(self.eyes_servo_state) < 2:
            self.eyes_servo_state = [0.0, 0.0]
        if not hasattr(self, "eyes_state") or len(self.eyes_state) < 2:
            self.eyes_state = [0.0, 0.0]

        for i, val in enumerate(pos[:2]):
            if math.isnan(val):
                self.eyes_servo_state[i] = self.eyes_center_position[i]
                self.logger.info(
                    f"Eye joint ID {self.eyes_joint_ids[i]} is not responding"
                )
                # keep previous camera state or center it, your choice:
                # self.eyes_state[i] = 0.0
                continue
            ...
            # Camera angle (scaled)
            if i == 0:
                coeff = self.camera_angle_eye_horizontal_coeff
            else:
                coeff = self.camera_angle_eye_vertical_coeff

            if coeff == 0:
                self.logger.error("Camera angle coefficient is zero; cannot scale.")
                continue

            self.eyes_state[i] = val / coeff
    ...
    def idle_timer_callback(self):
        ...
        max_travel_distance = abs(self.head_pan_lower_limit) + abs(
            self.head_pan_upper_limit
        )
        ...
        movement_time = int(
            max(abs(travel_distance) / max_idle_movement * 3000000000, 750000000)
        )
        ...
        if self.head_enabled:
            self.send_pan_and_vertical_tilt_goal(
                self.goal_pan,
                self.start_head_state[3],
                Duration(sec=0, nanosec=movement_time),
            )
        ...
        self.idle_timer.timer_period_ns = movement_time + random.uniform(
    ...
    def select_face_to_track(self, faces):
        ...
        if max_face_size > self.near_interaction_treshold:
            ...
            for face in faces:
                if (face.diagonal > self.ignore_small_faces_treshold) and (
                    face.face_id
                    not in [
                        blocked_face["face_id"] for blocked_face in self.blocked_faces
                    ]
                ):
        ...
        if selected_face:
            if (
                not self.previous_face
                or selected_face.face_id != self.previous_face.face_id
            ):
    ...
    def select_face_far_interaction(self, faces):
        if self.preferred_face_id:
            ...
            if selected_face:
                if (
                    time.time() - self.preferred_face_tracking_start_time
                    ...
                    if len(faces) > 1:
                        ...
                        face_list = [
                            face
                            for face in faces
                            if face.face_id != self.preferred_face_id
                        ]
    ...
    def update_blocked_faces(self):
        ...
        for blocked_face in self.blocked_faces:
            if (
                blocked_face["block_start"] + blocked_face["block_duration"]
                < time.time()
            ):
                self.logger.info(
    ...
    def track_face(self, face):
        ...
        if self.eyes_enabled and (
            horizontal_angle_eyes != 0 or vertical_angle_eyes != 0
        ):
            eye_goal_horizontal, eye_goal_vertical = (
                self.transform_camera_angle_to_eye_location(
    ...
    def nod(self, magnitude=0.4, delay=0.5, duration_of_individual_movements=0.4):
        ...
        if self.head_state:
            ...
            self.fixed_gaze_head_turn(
                "up", magnitude / 2, duration_of_individual_movements
            )
            time.sleep(delay)
            self.fixed_gaze_head_turn(
                "down", magnitude, duration_of_individual_movements
            )
            time.sleep(delay)
            verticalTilt = self.head_state[3]
            self.fixed_gaze_head_turn(
                "up",
                verticalTilt - verticalTilt_start,
                duration_of_individual_movements,
            )

    def head_shake(
        self, magnitude=0.5, delay=0.5, duration_of_individual_movements=0.4
    ):
        """
        Shakes the head.
        Takes two optional arguments:
        ...
        if self.head_state:
            ...
            # self.head_gesture_length_publisher.publish(msg)
            self.fixed_gaze_head_turn(
                "left", magnitude / 2, duration_of_individual_movements
            )
            time.sleep(delay)
            self.fixed_gaze_head_turn(
                "right", magnitude, duration_of_individual_movements
            )
            time.sleep(delay)
            pan = self.head_state[0]
            self.fixed_gaze_head_turn(
                "left", pan_start - pan, duration_of_individual_movements
            )
    ...
    def glance(self, delay=0.5, duration_of_individual_movements=None):
        ...
        """
        if self.head_state:
            eye_location_horizontal, eye_location_vertical = (
                self.get_random_eye_location(
    ...
    def fixed_gaze_head_turn(self, direction, magnitude, duration=0.4):
        ...
        if direction == "left":
            ...
            self.send_pan_and_vertical_tilt_goal(
                pan + magnitude, verticalTilt, duration
            )
            ...
            self.send_pan_and_vertical_tilt_goal(
                pan, verticalTilt - magnitude, duration
            )
    ...
    def send_eye_goal(self, horizontal, vertical, duration=None):
        ...
        if duration == None:
            ...
            duration = Duration(
                sec=0, nanosec=max(int(150000000 * horizontal_diff), 150000000)
            )
        ...
        self.logger.info(
            "eye location x: %f, eye location y: %f" % (horizontal, vertical)
        )

    def send_horizontal_tilt_goal(self, horizontalTilt):
        goal_msg = FollowJointTrajectory.Goal()
        trajectory_points = JointTrajectoryPoint(
            positions=[-horizontalTilt, horizontalTilt],
    ...
    def send_pan_and_vertical_tilt_goal(self, pan, verticalTilt, duration=None):
        ...
        goal_msg = FollowJointTrajectory.Goal()
        trajectory_points = JointTrajectoryPoint(
            positions=[pan * self.head_pan_multiplicator], time_from_start=duration
        )
        goal_msg.trajectory = JointTrajectory(
            joint_names=["head_pan_joint"], points=[trajectory_points]
        )
    ...
    def transform_camera_angle_to_eye_location(self, horizontal_angle, vertical_angle):
        ...
        eye_location_x = (
            horizontal_angle * self.camera_angle_eye_horizontal_coeff
            + self.eyes_servo_state[0]
        )
        # Vertical eye movement
        eye_location_y = (
            vertical_angle * self.camera_angle_eye_vertical_coeff
            + self.eyes_servo_state[1]
        )

        eye_location_y = max(
            min(self.eye_vertical_upper_limit, eye_location_y),
            self.eye_vertical_lower_limit,
        )
        eye_location_x = max(
            min(self.eye_horizontal_upper_limit, eye_location_x),
    ...
    def transform_camera_angle_to_head_location(self, horizontal_angle, vertical_angle):
        ...
        # Vertical tilt
        vertical_tilt = (
            vertical_angle * self.camera_angle_eye_vertical_coeff + self.head_state[3]
        )
        vertical_tilt = max(
            min(self.head_vertical_upper_limit, vertical_tilt),
            self.head_vertical_lower_limit,
        )
    ...
    def center_eyes(self, duration=None):
        self.send_eye_goal(
            self.eyes_center_position[0], self.eyes_center_position[1], duration
        )
    ...
    def get_random_eye_location(self, distance_from_current_horizontal_position=0):
        ...
        if (
            ...
            random_horizontal_list.append(
                random.uniform(
                    self.eye_horizontal_lower_limit,
                    self.eyes_servo_state[0]
                    - distance_from_current_horizontal_position,
                )
            )
        if (
        ...
        if len(random_horizontal_list) == 0:
            self.logger.warning(
                f"Cannot get random eye position due to too large {distance_from_current_horizontal_position =}"
