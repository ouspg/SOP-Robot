...
class FaceTrackerMovementNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        if randomvalue <= glance_percentage:
            eye_location_x, eye_location_y = get_random_eye_location()
        ...
        if self.pan_diff != 0 or self.v_diff != 0: 
            goal_pan = max(min(1.75, self.current_pan + self.pan_diff), -0.25) # limit head values to reasonable values
            goal_vertical_tilt = max(min(-0.2, self.current_vertical_tilt + self.v_diff), -1.0)
...
def main():
    ...
    action_client.send_head_goal(action_client.current_pan, action_client.current_vertical_tilt, 0.5)
    time.sleep(1)
