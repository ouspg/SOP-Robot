...
from face_tracker_msgs.msg import Faces, Occurance, Point2
...
class FaceTrackerNode(Node):
    def __init__(self):
        ...
        correlation_tracking = (
            self.declare_parameter('correlation_tracking', True).get_parameter_value()._bool_value
        )
        ...
        image_topic = (
            self.declare_parameter('image_topic', '/image_raw').get_parameter_value().string_value
        )
        ...
        if lip_movement_detection:
            self.predictor = dlib_api.shape_predictor(
                os.path.join(
