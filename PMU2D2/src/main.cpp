...
// Slave serial pins
const uint8_t SLAVE_RX = 10;
const uint8_t SLAVE_TX = 11;

// Baud rate, bps
const uint16_t MASTER_BAUD = 9600;
const uint32_t SLAVE_BAUD = 115200;  // 9600 57600 1152000

SoftwareSerial slaveSerial(SLAVE_RX, SLAVE_TX);

const uint16_t SLAVE_BUFF_SIZE = 255;
uint8_t SLAVE_BUFF[SLAVE_BUFF_SIZE] = {0};

const uint16_t MASTER_BUFF_SIZE = 255;
uint8_t MASTER_BUFF[MASTER_BUFF_SIZE] = {0};
...
void slaveComm() {
  int n_recv = slaveSerial.available();
  if (n_recv > 0) {
    if (n_recv >= static_cast<int>(sizeof(SLAVE_BUFF))) {
      n_recv = sizeof(SLAVE_BUFF) - 1;
    }

    slaveSerial.readBytes(SLAVE_BUFF, n_recv);
    SLAVE_BUFF[n_recv + 1] = '\0';
    Serial.println("Received: ");
    hexdump(SLAVE_BUFF, n_recv);
  } else {
    Serial.println("Send data to slave");

    // Broadcast (0xFE) ping all servos
    uint8_t msg[] = {0xFF, 0xFF, 0xFD, 0x00, 0xFE, 0x03, 0x00, 0x1, 0, 0};
    int CRC = update_crc(0, msg, sizeof(msg) - 2);
    msg[8] = CRC & 0xFF;
    msg[9] = (CRC >> 8) & 0xFF;

    slaveSerial.write(msg, sizeof(msg));
  }
}
